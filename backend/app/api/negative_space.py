...
def _phase_store(output_path: str | None) -> PhaseStore:
    return PhaseStore("phase7", Path(output_path) if output_path else DEFAULT_OUTPUT)


@router.post("/run")
def run_endpoint(request: NegativeSpaceRequest) -> dict[str, Any]:
    try:
        result = run_negative_space(request.input_path, request.output_path or DEFAULT_OUTPUT, request.dataset_id)
